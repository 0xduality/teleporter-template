...
interface ITeleporterMessenger {
    ...
    function checkRelayerRewardAmount(address relayer, address feeTokenAddress)
        external
        view
        returns (uint256);
    ...
    function getReceiptQueueSize(bytes32 sourceBlockchainID)
        external
        view
        returns (uint256);
    ...
    function retrySendCrossChainMessage(TeleporterMessage memory message)
        external;

    function sendCrossChainMessage(TeleporterMessageInput memory messageInput)
        external
        returns (bytes32);
